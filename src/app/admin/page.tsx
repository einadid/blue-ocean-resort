...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'add' | 'manage' | 'bookings' | 'users'>('dashboard')
  ...
  const handleAddRoom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('rooms').insert([{
        ...
        amenities: amenitiesArray.length > 0 ? amenitiesArray : ['WiFi', 'AC'],
      ...
      setFormData({ title: '', description: '', price: '', capacity: '2', amenities: '', image1: '', image2: '' })
      fetchRooms()
      setActiveTab('manage')
    } catch (error: unknown) {
      ...
      const { error } = await supabase.from('rooms').insert([
        {
          ...
          description: 'Spacious room with stunning ocean views and modern amenities. Perfect for couples.',
        ...
        },
        {
          ...
          amenities: ['WiFi', 'AC', 'TV', 'Kitchen', 'Living Room', '2 Bedrooms'],
          images: ['https://images.unsplash.com/photo-1578683010236-d716f9a3f461?w=800'],
          rating: 4.8, available: true,
        },
        {
          title: 'Beach Bungalow',
          description: 'Private bungalow right on the beach with direct beach access and private pool.',
          price: 25000, capacity: 3,
          amenities: ['WiFi', 'AC', 'TV', 'Beach Access', 'Private Pool', 'Kitchen'],
          images: ['https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=800'],
          rating: 5.0, available: true,
        },
        {
          title: 'Standard Room',
          description: 'Comfortable and affordable room with all basic amenities for budget travelers.',
        ...
        },
        {
          ...
          amenities: ['WiFi', 'AC', 'TV', 'Kitchen', 'Jacuzzi', 'Terrace', 'Butler Service'],
          images: ['https://images.unsplash.com/photo-1582719508461-905c673771fd?w=800'],
          rating: 5.0, available: true,
        },
        {
          title: 'Garden View Room',
          description: 'Peaceful room overlooking our beautiful tropical gardens. Perfect for nature lovers.',
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-8">
          ...
          <div className="bg-white p-4 rounded-xl shadow-md text-center">
            <div className="text-2xl font-bold text-red-600">{totalRooms - availableRooms}</div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              {allBookings.slice(0, 5).map((booking: any) => (
                <div key={booking.id} className="flex items-center justify-between py-3 border-b last:border-b-0">
                  <div>
                    <p className="font-semibold">{booking.rooms?.title || 'Room'}</p>
                    <p className="text-sm text-gray-500">
                      {booking.profiles?.username} • {new Date(booking.created_at).toLocaleDateString()}
                    </p>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className="font-bold text-ocean">৳{booking.total_price?.toLocaleString()}</span>
        ...
        {activeTab === 'manage' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-bold text-gray-900">All Rooms ({rooms.length})</h2>
              <button
                onClick={() => setActiveTab('add')}
              ...
              <div key={room.id} className="bg-white rounded-xl shadow-md overflow-hidden">
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="md:w-48 h-32 md:h-auto">
                    <img
                      src={room.images?.[0] || 'https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=400'}
                      alt={room.title}
                  ...
                  <div className="flex-grow p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-gray-500 text-sm mt-1">
                        ৳{room.price?.toLocaleString()}/night • {room.capacity} guests • ⭐ {room.rating}
                      </p>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => handleToggleRoom(room.id, room.available)}
